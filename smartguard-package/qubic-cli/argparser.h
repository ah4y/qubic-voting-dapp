...
void print_help()
{
    ...
    printf("\n[CONTRACT COMMANDS]\n");
    printf("\t-contractcompile <SOURCE_FILE> <OUTPUT_FILE>\n");
    printf("\t\tCompile a smart contract source file to bytecode. Generates bytecode for deployment.\n");
    printf("\t-contractvalidate <BYTECODE_FILE>\n");
    printf("\t\tValidate a smart contract bytecode file. Checks structure and basic integrity.\n");
    printf("\t-contractdeploy --bytecode <FILE> --wallet <NAME> --network <NETWORK> [--gas-limit <LIMIT>] [--gas-price <PRICE>]\n");
    printf("\t\tDeploy a smart contract bytecode to the specified network.\n");
    ...
    printf("\n[TESTNET COMMANDS]\n");
    printf("\t-faucet request --address <WALLET_ADDRESS> --network testnet\n");
    printf("\t\tRequest testnet tokens from the faucet for testing purposes.\n");
    printf("\t-wallet balance --name <WALLET_NAME> --network testnet\n");
    printf("\t\tCheck wallet balance on the specified network.\n");
}
...
void parseArgument(int argc, char** argv)
{
    ...
    while (i < argc)
    {
        ...
        if (strcmp(argv[i], "-contractcompile") == 0) {
            CHECK_NUMBER_OF_PARAMETERS(2)
            g_cmd = CONTRACT_COMPILE;
            strcpy(g_contract_source_file, argv[i + 1]);
            strcpy(g_contract_output_file, argv[i + 2]);
            i += 3;
            CHECK_OVER_PARAMETERS
            break;
        }
        
        if (strcmp(argv[i], "-contractvalidate") == 0) {
            CHECK_NUMBER_OF_PARAMETERS(1)
            g_cmd = CONTRACT_VALIDATE;
            strcpy(g_contract_bytecode_file, argv[i + 1]);
            i += 2;
            CHECK_OVER_PARAMETERS
            break;
        }
        
        if (strcmp(argv[i], "-contractdeploy") == 0) {
            // Parse contract deployment with named parameters
            g_cmd = CONTRACT_DEPLOY;
            i++; // Move past -contractdeploy
            
            // Parse all the named parameters
            while (i < argc) {
                if (strcmp(argv[i], "--bytecode") == 0) {
                    if (i + 1 >= argc) {
                        LOG("Error: --bytecode parameter requires a value\n");
                        exit(1);
                    }
                    strcpy(g_deploy_bytecode_file, argv[i + 1]);
                    i += 2;
                } else if (strcmp(argv[i], "--wallet") == 0) {
                    if (i + 1 >= argc) {
                        LOG("Error: --wallet parameter requires a value\n");
                        exit(1);
                    }
                    g_deploy_wallet_name = argv[i + 1];
                    i += 2;
                } else if (strcmp(argv[i], "--network") == 0) {
                    if (i + 1 >= argc) {
                        LOG("Error: --network parameter requires a value\n");
                        exit(1);
                    }
                    g_deploy_network = argv[i + 1];
                    i += 2;
                } else if (strcmp(argv[i], "--gas-limit") == 0) {
                    if (i + 1 >= argc) {
                        LOG("Error: --gas-limit parameter requires a value\n");
                        exit(1);
                    }
                    g_deploy_gas_limit = charToUnsignedNumber(argv[i + 1]);
                    i += 2;
                } else if (strcmp(argv[i], "--gas-price") == 0) {
                    if (i + 1 >= argc) {
                        LOG("Error: --gas-price parameter requires a value\n");
                        exit(1);
                    }
                    g_deploy_gas_price = charToUnsignedNumber(argv[i + 1]);
                    i += 2;
                } else {
            ...
            if (strlen(g_deploy_bytecode_file) == 0) {
                LOG("Error: --bytecode parameter is required for contract deployment\n");
                exit(1);
            }
            if (g_deploy_wallet_name == nullptr) {
                LOG("Error: --wallet parameter is required for contract deployment\n");
                exit(1);
            }
            if (g_deploy_network == nullptr) {
                LOG("Error: --network parameter is required for contract deployment\n");
                exit(1);
            }
        ...
        if (strcmp(argv[i], "-faucet") == 0) {
            if (i + 1 < argc && strcmp(argv[i + 1], "request") == 0) {
                CHECK_NUMBER_OF_PARAMETERS(5)  // -faucet request --address ADDRESS --network NETWORK
                g_cmd = FAUCET_REQUEST;
                
                // Parse --address parameter
                if (i + 2 < argc && strcmp(argv[i + 2], "--address") == 0) {
                    g_faucetAddress = argv[i + 3];
                } else {
                    LOG("Error: --address parameter required for faucet request\n");
                    exit(1);
                }
                
                // Parse --network parameter  
                if (i + 4 < argc && strcmp(argv[i + 4], "--network") == 0) {
                    g_networkType = argv[i + 5];
                } else {
                    LOG("Error: --network parameter required for faucet request\n");
                    exit(1);
                }
        ...
        if (strcmp(argv[i], "-wallet") == 0) {
            if (i + 1 < argc && strcmp(argv[i + 1], "balance") == 0) {
                CHECK_NUMBER_OF_PARAMETERS(5)  // -wallet balance --name NAME --network NETWORK
                g_cmd = WALLET_BALANCE;
                
                // Parse --name parameter
                if (i + 2 < argc && strcmp(argv[i + 2], "--name") == 0) {
                    g_walletName = argv[i + 3];
                } else {
                    LOG("Error: --name parameter required for wallet balance\n");
                    exit(1);
                }
                ...
                if (i + 4 < argc && strcmp(argv[i + 4], "--network") == 0) {
                    ...
                    LOG("Error: --network parameter required for wallet balance\n");
                    exit(1);
                }
        ...
        if (strcmp(argv[i], "-contractcall") == 0) {
            g_cmd = CONTRACT_CALL;
            
            // Parse required --contract parameter
            if (i + 1 < argc && strcmp(argv[i + 1], "--contract") == 0) {
                g_call_contract_address = argv[i + 2];
                i += 2;
            } else {
                LOG("Error: --contract parameter required for contract call\n");
                exit(1);
            }
            
            // Parse required --function parameter
            if (i + 1 < argc && strcmp(argv[i + 1], "--function") == 0) {
                g_call_function_name = argv[i + 2];
                i += 2;
            } else {
                LOG("Error: --function parameter required for contract call\n");
                exit(1);
            }
            
            // Parse required --wallet parameter
            if (i + 1 < argc && strcmp(argv[i + 1], "--wallet") == 0) {
                g_call_wallet_name = argv[i + 2];
                i += 2;
            } else {
                LOG("Error: --wallet parameter required for contract call\n");
                exit(1);
            }
            
            // Parse required --network parameter
            if (i + 1 < argc && strcmp(argv[i + 1], "--network") == 0) {
                g_call_network = argv[i + 2];
                i += 2;
            } else {
                LOG("Error: --network parameter required for contract call\n");
                exit(1);
            }
