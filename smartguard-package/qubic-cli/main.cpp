...
int run(int argc, char* argv[])
{
    ...
    switch (g_cmd)
    {
        ...
        case CONTRACT_COMPILE: {
            compileContract(g_contract_source_file, g_contract_output_file);
            break;
        }
        case CONTRACT_VALIDATE: {
            validateContract(g_contract_bytecode_file);
            break;
        }
        case CONTRACT_DEPLOY: {
            deployContract(g_deploy_bytecode_file, g_deploy_wallet_name, g_deploy_network, g_deploy_gas_limit, g_deploy_gas_price);
            break;
        }
        case CONTRACT_CALL: {
            callContract(g_call_contract_address, g_call_function_name, g_call_wallet_name, g_call_network, g_call_function_args, g_call_gas_limit, g_call_gas_price);
            break;
        }
        case FAUCET_REQUEST: {
            requestFaucetTokens(g_faucetAddress, g_networkType);
            break;
        }
        case WALLET_BALANCE: {
